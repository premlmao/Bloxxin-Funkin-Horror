...
function onCreate()
{
	incredibleBlackScreen = new FlxSprite().makeGraphic(FlxG.width ,FlxG.height , FlxColor.BLACK); //Added this here due to timing reasons in the song for some reason
    incredibleBlackScreen.scrollFactor.set();
    incredibleBlackScreen.screenCenter();
    add(incredibleBlackScreen);
...
function killme()
{
	incredibleBlackScreen.visible = true;
	modchartTweens.set('ooofadeout', FlxTween.tween(incredibleBlackScreen, {alpha: 0}, 1.5, {onComplete: twn -> {
