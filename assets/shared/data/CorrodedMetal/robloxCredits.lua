...
-----------------musicians--------------------
	...
	makeLuaText("musicians", 'Coolert', 0, 170, 250); --change this to musicians name
...
-----------------artists--------------------
	...
	makeLuaText("eventsText", '[events]:', 0, 20, 310);
	addLuaText('eventsText');
	setTextSize('eventsText', 20);
	setTextColor("eventsText", "FF0000")
	setObjectCamera('eventsText', 'hud');

	makeLuaText("events", 'Prem1um', 0, 140, 310); --change this to artists name
	addLuaText('events');
	setTextSize('events', 20);
	setObjectCamera('events', 'hud');


-----------------quote lol--------------------

	makeLuaText("quote", 'R u still there? :)', 0, 20, 385); --quote editing here
	addLuaText('quote');
...
function onTimerCompleted(tag, loops, loopsLeft)
	...
	doTweenX('bye9', 'eventsText', -1000, 0.8, 'Linear');
	doTweenX('bye10', 'events', -1000, 0.8, 'Linear');

	doTweenX('bye11', 'quote', -1000, 0.8, 'Linear');
    end
end
...
function onTweenCompleted(tag)
    ...
    removeLuaSprite('eventsText', true)
    removeLuaSprite('events', true)
