...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['####', 0.2], //From 0% to 19%
		['Noob', 0.4], //From 20% to 39%
		['Bad', 0.5], //From 40% to 49%
		['Mid', 0.6], //From 50% to 59%
		['Okay', 0.69], //From 60% to 68%
		['Awesome', 0.7], //69%
		['Fantastic', 0.8], //From 70% to 79%
		['Bloxxin', 0.9], //From 80% to 89%
		['Pro', 1], //From 90% to 99%
		['Exploiter', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		timeTxt.setFormat(Paths.font("Gotham Black Regular.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		healthBarAround = new FlxSprite(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), Paths.image('healthBarAround'));
		...
		scoreTxt.setFormat(Paths.font("Gotham Black Regular.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override public function update(elapsed:Float)
	{
		...
		{
			if (FlxG.keys.justPressed.SEVEN && FlxG.keys.pressed.CONTROL)
				openChartEditor();
			else if (FlxG.keys.justPressed.EIGHT && FlxG.keys.pressed.CONTROL)
				openCharacterEditor();
		}
	...
	public dynamic function updateIconsPosition()
	{
		...
		/*
		if(curStage.toLowerCase() == 'crossroads')
			{
				{
			if (healthBar.percent > 80)
				iconP2.animation.play('defeated');
			else
